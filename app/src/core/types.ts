...
export interface Nutrients {
  ...
  fiber?: number;
  /**
   * Органические кислоты, г (лимонная, яблочная, молочная).
   *
   * Зачем поле нужно. Справочник Скурихина считает калорийность
   * по расширенной схеме Атуотера: 4/9/4 плюс 3 ккал/г на органические
   * кислоты. Без этого слагаемого лимон «не сходится» на 51%:
   * 0.9×4 + 0.1×9 + 3.0×4 = 16.5 ккал против заявленных 34.
   * С кислотами (5.7 г × 3 = 17.1) получается 33.6 — ровно справочное
   * значение. Данные верны, неполной была наша формула проверки.
   *
   * Заполняется только там, где величина значима (>0.5 г/100 г):
   * цитрусовые, ягоды, кисломолочное, квашеные овощи.
   */
  organicAcids?: number;
}
...
export interface EaterProfile {
  ...
  goal: Goal;
  /**
   * Целевой белок, г на кг массы тела. Задаётся НЕЗАВИСИМО от калорий.
   *
   * Если не задан — берётся норма по цели (1.2 / 1.6 / 1.8).
   * Смысл отдельного параметра: поднять белок, не поднимая калорийность.
   * Раньше это было невозможно, и человек ради белка ставил цель
   * «набрать вес», получая вместе с белком лишние 1100 ккал.
   */
  proteinPerKg?: number;
