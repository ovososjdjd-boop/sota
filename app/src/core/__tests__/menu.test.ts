...
describe('база рецептов: целостность', () => {
  ...
  it('цена порции в разумных пределах', () => {
    // Потолок зависит от класса блюда, а не единый для всех.
    //
    // Раньше стояло жёсткое «меньше 250 ₽», и это было правильно,
    // пока база состояла из повседневной еды. С появлением блюд
    // для свободного режима (лосось, креветки, стейк) порог начал
    // ловить не ошибки, а замысел: премиальное блюдо и должно быть
    // дорогим, иначе оно не премиальное.
    //
    // Но совсем убирать проверку нельзя — она ловит реальные дефекты
    // данных (форель за 278 ₽ из-за неверной доли отходов). Поэтому
    // потолок разный: обычное блюдо дороже 250 ₽ — почти наверняка
    // ошибка, деликатес — нет.
    for (const recipe of RECIPES) {
      const stats = computeRecipeStats(recipe);
      const limit = recipe.tags.includes('delicacy') ? 400 : 250;
      expect(stats.cost, recipe.name).toBeGreaterThan(0);
      expect(stats.cost, `${recipe.name} (лимит ${limit} ₽)`).toBeLessThan(limit);
    }
  });
...
describe('планировщик меню', () => {
  ...
  it('размещает подавляющее большинство заказанных порций', async () => {
    ...
    // 10%: полные 100% недостижимы принципиально — правило неповторения
    // и структура приёмов ограничивают вместимость расписания.
    // Важно, что неразложенное больше не покупается: корзина считается
    // по расписанию, поэтому остаток не бьёт ни по деньгам, ни по КБЖУ.
    expect(unplaced / total).toBeLessThan(0.1);
  }, 90000);
