  ...
  // Ставим редко: не чаще раза в три дня, это радость, а не рацион.
  {
    ...
    for (const entry of remaining.values()) {
      ...
      for (const day of order) {
        ...
        //
        // Десерт размещается первым (иначе ему не достаётся места
        // вовсе), и потому обязан быть скромным: «первым» не значит
        // «в ущерб ужину». Резервируем под основную еду большую часть
        // дневного бюджета времени.
        //
        // Половина, а не треть. Треть ставилась, когда причиной
        // голодных дней считали десерт; настоящей причиной оказалась
        // лишняя переготовка (см. награду за доедание кастрюли выше).
        // Слишком жёсткий лимит просто выкидывал десерты из плана,
        // не решая ничего.
        const dessertBudget = maxMinutesPerDay
          ? Math.floor(maxMinutesPerDay / 2)
          : undefined;
    ...
    while (progress && guard++ < 3000) {
      ...
      for (const day of schedule) {
        for (const meal of day.meals) {
          ...
          if (ratio >= stage.fillTo || dayRatio >= stage.dayCap) continue;

          // НЕ РАЗДУВАЕМ СЫТЫЙ ПРИЁМ, ПОКА РЯДОМ ГОЛОДНЫЙ.
          //
          // НАЙДЕННЫЙ ДЕФЕКТ. Уборщик разрешал приёму дорасти до 135%
          // нормы, глядя только на него и на день целиком. В итоге
          // завтрак раздувался до 110%, а перекус того же дня стоял
          // на 62% — еда была, место было, но алгоритм считал завтрак
          // «ещё не полным» и продолжал носить туда.
          //
          // Человек ест четыре раза в день, а не один раз побольше.
          // Перебирать приём допустимо, только когда остальным уже
          // хватает: иначе это не «доедание остатков», а перекос.
          const hungrierNearby = day.meals.some(
            (other) =>
              other !== meal &&
              other.nutrients.kcal / Math.max(1, other.targetKcal) < 0.85,
          );
          if (ratio >= 1 && hungrierNearby) continue;
...
function findCookDay(
  ...
  for (let d = dayIndex - 1; d >= earliest; d--) {
    ...
    if (load + recipe.minutes > maxMinutesPerDay) continue;

    // ── ДОНОР САМ ДОЛЖЕН ПОЕСТЬ ──
    //
    // НАЙДЕННЫЙ ДЕФЕКТ (scripts/day1c.ts). Готовка впрок записывалась
    // в любой день, где хватало минут, — включая день, который ещё
    // не наполнен едой. Замер: день 1 варил 30 минут для дня 2,
    // упирался в лимит 90 и оставался с пустыми ужином и перекусом —
    // 898 ккал вместо 2506. Человек в первый же день плана голодает,
    // чтобы во второй было удобнее.
    //
    ...
    if (donorFill < 0.7 && donorTimeLeft < maxMinutesPerDay * 0.25) continue;
...
function pickDishForMeal(
  ...
  for (const entry of remaining.values()) {
    ...
    if (served.length > 0) {
      ...
      // ── КАСТРЮЛЯ ДОЛЖНА ДОЖИТЬ ДО ВТОРОЙ ПОДАЧИ ──
      //
      // НАЙДЕННЫЙ ДЕФЕКТ (scripts/reheat.ts). Награда за разрыв выше
      // толкала раскладку разносить подачи как можно дальше: гороховый
      // суп шёл в дни 4, 11, 13, 23, 30 и варился ЧЕТЫРЕ раза вместо
      // двух, хотя хранится три дня и кастрюли хватает на шесть порций.
      // Суммарно на переготовку уходило 615 лишних минут — почти
      // семь дневных бюджетов времени.
      //
      // Следствие было тяжёлым: время кончалось, приёмы оставались
      // пустыми, и появлялись дни на 898-1600 ккал. Симптом выглядел
      // как «раскладка не наполняет дни», а причина была в том, что
      // она сама сжигала бюджет времени на лишнюю готовку.
      //
      // Разнообразие и батч-кукинг конфликтуют только на первый взгляд.
      // Правильный компромисс: подать блюдо ВТОРОЙ раз, пока готовое
      // ещё не испортилось, — это и разнообразию не мешает (второй раз
      // за период, а не десятый), и время экономит вдвое.
      if (recipe.keepsDays > 0 && gap <= recipe.keepsDays) {
        score += 0.55;
      }
    } else {
      score += 0.75;
    ...
    // ── УНИВЕРСАЛЬНОЕ БЛЮДО НЕ ЗАНИМАЕТ ЧУЖОЕ МЕСТО ──
    //
    // НАЙДЕННЫЙ ДЕФЕКТ (scripts/snackwhere.ts). Блины, оладьи, йогурт
    // с овсянкой годятся и в завтрак, и в перекус. Завтрак разбирается
    // раньше — и забирал их себе целиком: 11 470 ккал перекусной еды
    // уходило в другие приёмы, а перекус оставался с чашкой кофе
    // на 84 ккал. Восемь дней из тридцати он был заполнен на 22%.
    //
    // Заказ при этом был правильный. Симптом читался как «раскладка
    // теряет порции», хотя терять было нечего: еду просто съедал
    // сосед по расписанию.
    //
    // Правило простое и общее: чем в большее число приёмов годится
    // блюдо, тем ниже его приоритет в каждом конкретном. Специалист
    // идёт вперёд универсала — универсал найдёт себе место и потом.
    // Это не про перекус: то же самое спасает ужин от обеда.
    score += 0.35 / recipe.slots.length;
