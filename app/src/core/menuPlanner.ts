...
function buildDishModel(
  ...
  if (maxMinutesPerDay && maxMinutesPerDay > 0) {
    ...
    // в точку насыщения.
    // В ЭКОНОМНОМ РЕЖИМЕ РЕЗЕРВ БОЛЬШЕ.
    //
    // Найденный дефект: при 11 000 ₽ на месяц на тарелку попадало 91%
    // заказанного против 97-98% при больших бюджетах, и появлялись
    // голодные дни. Причина не в деньгах, а во времени: дешёвая еда
    // трудоёмка. Замер (scripts/leantime.ts) — 62.0 минуты на 1000 ккал
    // против 56.9 у сбалансированного режима. Крупы, супы и бобовые
    // надо варить, а готовое и дорогое — нет.
    //
    // Значит и запас на упаковку по дням нужен больше: неделимых
    // «кусков» времени столько же, а каждый крупнее.
    const PACKING_RESERVE = modeProfile.mode === 'lean' ? 0.84 : 0.88;
    lines.push(
